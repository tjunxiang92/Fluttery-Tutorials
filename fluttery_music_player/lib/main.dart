...
import 'package:fluttery_music_player/bottom_controls.dart';
import 'package:fluttery_music_player/songs.dart';
import 'package:fluttery_music_player/theme.dart';
            ...
            new Expanded(
              child: new AudioPlaylistComponent(
                playlistBuilder: (BuildContext context, Playlist playlist, Widget child) {
                  final albumArtUrl = demoPlaylist.songs[playlist.activeIndex].albumArtUrl;
...
class VisualizerPainter extends CustomPainter {
  ...
  void _renderHistogram(Canvas canvas, Size size, List<int> histogram) {
    ...
    for (int i = 0; i < histogram.length - 1; i++) {
      ...
      // 0.0 is a high point, 125.0 is low point
  ...
  List<int> _createHistogram(List<int> samples, int bucketCount, [int start, int end]) {
    ...
    // Remove the leftover buckets after all are filled
    ...
    for (int i = start; i <= start + actualSampleCount; i++) {
      // Ignore the odd number as that repres. the imaginary part
      if ((i - start) % 2 == 1) {
        continue;
      }
...
class RadialSeekBarState extends State<RadialSeekBar> {
  ...
  void _onDragStart(PolarCoord coord) {
    ...
    _currentDragPercent = _startDragPercent;
  }

  void _onDragUpdate(PolarCoord coord) {
    final dragAngle = coord.angle - _startDragCoord.angle;
    final dragPercent = dragAngle / (pi * 2);
...
class AudioRadialSeekBar extends StatefulWidget {
  ...
  @override
  _AudioRadialSeekBarState createState() => new _AudioRadialSeekBarState();
}
